...
const RecipePage = () => {
    ...
    if (error) return (
        <div className="container mx-auto px-4 py-8">
            <div className="text-red-500">Error loading recipe: {error.message}</div>
        </div>
    )
            ...
            <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-6 items-center mb-8">
                    <div className="flex items-center gap-2">
                        <Heart weight="fill" className="text-red-500" size={20} />
                        <span className="text-gray-700">{recipe.favorites_count}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Star weight="fill" className="text-yellow-500" size={20} />
                        <span className="text-gray-700">{recipe.ratings_avg?.toFixed(1) || '0.0'}</span>
                        <span className="text-gray-500">({recipe.ratings_count})</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Users weight="fill" className="text-blue-500" size={20} />
                        <span className="text-gray-700">{recipe.servings} servings</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Clock weight="fill" className="text-green-500" size={20} />
                        <span className="text-gray-700">
                            {formatTime(recipe.cook_time_hours, recipe.cook_time_minutes)}
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Clock weight="fill" className="text-purple-500" size={20} />
                        <span className="text-gray-700">
                            {formatTime(recipe.prep_time_hours, recipe.prep_time_minutes)}
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
